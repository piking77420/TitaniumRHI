...
namespace TiRHI::Vulkan
{
    ...
    static std::unordered_map<VkResult, std::string> ErrorDescriptions = {
        ...
        {VK_THREAD_IDLE_KHR, "A deferred operation is not complete but there is currently no work for this thread "
                             "to do at the time of this call."},
        {VK_THREAD_DONE_KHR,
        ...
        {VK_ERROR_FRAGMENTED_POOL,
         ...
         "returned in preference to VK_ERROR_OUT_OF_POOL_MEMORY, but only if the implementation is certain that "
        ...
        {VK_ERROR_INVALID_OPAQUE_CAPTURE_ADDRESS,
         ...
         "group handle assignment failed because the requested shader group handle information is no longer "
         "valid."},
        {VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT,
         "An operation on a swapchain created with VK_FULL_SCREEN_EXCLUSIVE_APPLICATION_CONTROLLED_EXT failed as "
         "it did not have exlusive full-screen access. This may occur due to implementation-dependent reasons, "
...
#ifdef NDEBUG
#define VK_CHECK_CALL(x) VulkanCheckErrorStatus(x, __LINE__, FUNCTION_NAME, FILENAME)
#else
#define VK_CHECK_CALL(x)                                                                                               \
    if constexpr (ENABLE_VALIDATION_LAYERS)                                                                            \
    VulkanCheckErrorStatus(x, __LINE__, FUNCTION_NAME, FILENAME)
#endif

    static inline bool VulkanCheckErrorStatus(vk::Result _x, int _lign, const char* _func, const char* _file)
    {
